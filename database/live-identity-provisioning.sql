...
-- This table is intentionally absent from ordinary browser Data API grants.
revoke all privileges on public.account_invitations from public, anon, authenticated;

-- ---------------------------------------------------------------------------
-- Issuer-authorisation helper. Service-role caller remains subject to the
-- explicit actor checks in this function; browser roles cannot execute it.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Platform administrators may issue any pilot role except historical roles,
  -- which remain a separate transition/verification workflow.
  if v_actor.primary_role = 'platform_admin' then
    if p_intended_role in ('parent_alumni', 'alumni', 'mentor', 'organisation_admin') then
    ...
    if p_intended_role <> 'child' then
      raise exception 'parent may invite a child only';
    end if;
    if p_school_id is not null and not exists (
      ...
      raise exception 'parent school scope mismatch';
    end if;
  ...
  if p_cohort_id is not null and p_school_id is null then
    raise exception 'cohort requires school';
  end if;
...
-- ---------------------------------------------------------------------------
-- Claim is atomic: profile + membership + optional guardian relationship +
-- audit + invitation state commit together or not at all.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if exists (select 1 from public.profiles p where p.auth_user_id = p_auth_user_id) then
    raise exception 'profile already exists';
  end if;

  select ai.* into v_invitation
  from public.account_invitations ai
  where ai.token_hash = lower(p_token_hash)
  ...
  insert into public.profiles (
    ...
    v_invitation.intended_age_band,
    p_country_code
  ...
  if v_invitation.intended_role = 'child' and v_invitation.guardian_sponsor_profile_id is not null then
    insert into public.guardian_links(
      ...
      v_invitation.guardian_sponsor_profile_id,
      v_profile_id,
      'guardian',
      'verified',
      now()
...
create function public.server_revoke_account_invitation(
  p_auth_user_id uuid,
...
begin
  ...
  if v_actor_id is null or v_invitation.id is null then
    return false;
  end if;
  if v_invitation.issued_by_profile_id <> v_actor_id then
    raise exception 'invitation issuer mismatch';
  end if;
...
-- Server-only functions. The Edge workflow receives authenticated identity and
-- passes auth-user UUIDs; ordinary browser roles cannot call these RPCs.
