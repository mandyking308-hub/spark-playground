  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Badge variant="outline">Least-privilege access</Badge>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Every action keeps a reason, reviewer and timestamp.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2 text-sm">
            {["Automated safety signal", "Human triage", "Escalation where required", "Decision and action", "Appeal / review", "Audit record"].map((step, index) => (
              <div key={step} className="flex items-center gap-3 rounded-lg border p-3"><span className="text-xs font-medium text-muted-foreground">0{index + 1}</span><span>{step}</span></div>
            ))}
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2"><ShieldCheck className="size-5 text-primary" /><CardTitle>Access boundary</CardTitle></div>
            <CardDescription>Safeguarding records are more sensitive than normal school administration.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            <p className="rounded-lg bg-muted/40 p-3">✓ Teachers see only the cases their role and school scope permit.</p>
            <p className="rounded-lg bg-muted/40 p-3">✓ School and group escalation scopes remain tenant-bound.</p>
            <p className="rounded-lg bg-muted/40 p-3">✕ Parent, alumni and organisation roles do not enter this control plane.</p>
          </CardContent>
        </Card>
      </section>
    </div>
