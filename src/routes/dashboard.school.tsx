...
import { BarChart3, BrainCircuit, Building2, ShieldCheck, UserCog, UsersRound } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <Card><CardHeader><UsersRound className="size-5 text-primary" /><CardTitle>People & cohorts</CardTitle><CardDescription>School-scoped staff, learners and verified family links.</CardDescription></CardHeader><CardContent><Button asChild variant="outline"><Link to="/dashboard/school-people">Open people</Link></Button></CardContent></Card>
        <Card><CardHeader><UserCog className="size-5 text-primary" /><CardTitle>Staff roles</CardTitle><CardDescription>Assign verified roles without privilege escalation.</CardDescription></CardHeader><CardContent><Button asChild variant="outline"><Link to="/dashboard/staff-roles">Manage roles</Link></Button></CardContent></Card>
      ...
      <Card><CardHeader><div className="flex items-center gap-2"><BarChart3 className="size-5 text-primary" /><CardTitle>School reporting</CardTitle></div><CardDescription>Participation, project creation, challenge completion, passport achievements and safeguarding trends roll up here. Child-level access remains school-scoped and purpose-limited.</CardDescription></CardHeader></Card>
    </div>
  );
}
