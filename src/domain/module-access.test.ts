...
describe("core module audience boundaries", () => {
  test("children can access creation, shows, discovery, passport, challenge and club modules", () => {
    expect(canAccessCoreModule("child", "creator_studio")).toBe(true);
    expect(canAccessCoreModule("child", "shows")).toBe(true);
  ...
  test("adults cannot enter protected child shows or discovery modules", () => {
    for (const role of ["parent", "parent_alumni", "organisation_admin", "alumni"] as const) {
      expect(canAccessCoreModule(role, "shows")).toBe(false);
      expect(canAccessCoreModule(role, "discovery")).toBe(false);
    }
  });
  ...
  test("modules expose their security audience explicitly", () => {
    ...
    expect(coreModuleAudience("shows")).toBe("protected_child");
